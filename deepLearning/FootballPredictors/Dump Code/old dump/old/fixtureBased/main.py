...
def calculate_other_averages(df, home_team_encoded, away_team_encoded, other_columns):
    """Calculate weighted averages of other columns by combining home and away team stats."""
    home_team_stats = df[df['HomeTeam'] == home_team_encoded][other_columns]
    away_team_stats = df[df['AwayTeam'] == away_team_encoded][other_columns]
    
    # Combine both home and away stats by averaging
    combined_stats = (home_team_stats.mean() + away_team_stats.mean()) / 2
...
def prepare_data(file_path, decay_factor=0.1):
    ...
    return features_scaled, target, label_encoder_home, label_encoder_away, scaler, home_stats, away_stats, other_columns, match_results
...
def predict_match_result(model, scaler, label_encoder_home, label_encoder_away, home_stats, away_stats, other_columns, match_results, home_team, away_team):
    ...
    # Calculate the other columns as the average of home and away team stats for 'other_columns'
    other_stats = calculate_other_averages(match_results, home_team_encoded, away_team_encoded, other_columns)

    # Combine all stats into a single DataFrame for prediction
    # Ensure that all elements are lists to avoid scalar issues
    example_match = pd.DataFrame({
        'HomeTeam': [home_team_encoded],
        'AwayTeam': [away_team_encoded],
        **{col: [home_stats_row[col]] for col in home_stats_row.index},
        **{col: [away_stats_row[col]] for col in away_stats_row.index},
        **{col: [other_stats[col]] for col in other_stats.index}
    })
...
if __name__ == "__main__":
    ...
    features_scaled, target, label_encoder_home, label_encoder_away, scaler, home_stats, away_stats, other_columns, match_results = prepare_data('final.csv', decay_factor=0.1)
    ...
    outcome_labels, predicted_probabilities = predict_match_result(
        model, scaler, label_encoder_home, label_encoder_away, home_stats, away_stats, other_columns, match_results, home_team, away_team
    )
